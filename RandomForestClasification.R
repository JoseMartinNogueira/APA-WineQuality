...
#white wine
??rfcv
wwModelRF <- rfcv(whiteWine.learn.input, whiteWine.learn.classes, cv.fold =10)
summary(wwModelRF)
wwModelRF$error.cv
...
rwModelRF <- rfcv(redWine.learn.input, redWine.learn.classes, cv.fold =10)
rwModelRF$error.cv

(rwNtrees <- round(10^seq(1,3,by=0.2)))

rwRFResults <- matrix(rep(0,2*length(rwNtrees)),nrow=length(rwNtrees))
colnames(rwRFResults) <- c("ntrees", "OOB")
rwRFResults[,"ntrees"] <- rwNtrees
rwRFResults[,"OOB"] <- 0
...
for(nt in rwNtrees) {
  print(nt)
  rwModelRF <- randomForest(quality ~., data = redWine.learn, ntree=nt, proximity=FALSE)
  rwRFResults[ii,"OOB"] <- rwModelRF$err.rate[nt,1]
  ii <- ii+1
}

rwRFResults

rwlowesOOB.error <- as.integer(which.min(rwRFResults[,"OOB"]))
(rwBestNtrees <- rwRFResults[rwlowesOOB.error,"ntrees"])
